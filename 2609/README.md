...
- 順位は **307位 / 2,687チーム(上位11.4%)** — 2026-09-22 時点の暫定。締切は 2026-09-30
...
| モデル | OOF AUC | Public LB | 順位(このスコアなら現時点で) |
|---|---|---|---|
| LightGBM | 0.94123 | 0.94093 | 2229位(上位83%) |
| XGBoost | 0.94124 | 0.94152 | 1843位(上位69%) |
| CatBoost | 0.94156 | 0.94170 | 1608位(上位60%) |

> 順位は 2026-09-22 時点(2,688チーム)の LB で、そのスコアが何位に相当するかを示したもの。
...
(**307位 / 2,687チーム**、2026-09-22 時点の暫定)
...
|---|---|---|---|
| CatBoost 単体(ベースライン) | 0.94156 | 0.94170 | 1608位(上位60%) |
| FE + 収束確認 | 0.94555 | 0.94576 | 943位(上位35%) |
| Triple TE + digit | 0.94604 | 0.94622 | 672位(上位25%) |
| **列サブサンプリング + 3モデル** | **0.94623** | **0.94645** | **283位(上位10.5%)** |

> 順位はいずれも 2026-09-22 時点(2,688チーム)の LB に照らした相当位置。
> 実際の現在順位は **307位**(同スコアの中での並び順の差)。
> 参加チームは増え続けるため、同じスコアでも順位は下がっていく。
