...
- 順位は **266位 / 2,607チーム(上位10.2%)** — 2026-09-21 時点の暫定。1位は 0.94675、締切は 2026-09-30
...
|---|---|---|---|
| LightGBM | 0.94123 | 0.94093 | 2168位(上位83%) |
| XGBoost | 0.94124 | 0.94152 | 1791位(上位69%) |
| CatBoost | 0.94156 | 0.94170 | 1558位(上位60%) |

> 順位は 2026-09-21 時点(2,607チーム)の LB で、そのスコアが何位に相当するかを示したもの。
...
(**266位 / 2,607チーム**、2026-09-21 時点の暫定)
...
|---|---|---|---|
| CatBoost 単体(ベースライン) | 0.94156 | 0.94170 | 1558位(上位60%) |
| FE + 収束確認 | 0.94555 | 0.94576 | 911位(上位35%) |
| Triple TE + digit | 0.94604 | 0.94622 | 641位(上位25%) |
| **列サブサンプリング + 3モデル** | **0.94623** | **0.94645** | **266位(上位10.2%)** |

> 順位はいずれも 2026-09-21 時点(2,607チーム)の LB に照らした相当位置。
