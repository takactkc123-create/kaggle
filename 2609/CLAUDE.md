...
- 目標は**リーダーボード上位15%以内**。2026-09-21 時点で **266位 / 2,607チーム(上位10.2%)** と暫定的に圏内。
  **参加チームは増え続けるため、同じスコアでも順位は下がる。**
- ~~当面の目標: CV AUC = 0.95~~ → **撤回**。較正した確率からラベルを再サンプリングして推定した
  「完璧なモデルのAUC」は **0.9459 ± 0.0003**。LB1位が 0.94675 で既に理論上限付近であり到達不能。
...
| 順位(2026-09-21 時点の暫定) | **266位 / 2,607チーム(上位10.2%)** |
| 提出構成 | **LightGBM 1/3 + XGBoost 1/3 + RealMLP 1/3**(rank平均) |
| LB1位 | 0.94675(差 0.00030) |
