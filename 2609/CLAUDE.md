...
- 目標は**リーダーボード上位15%以内**。2026-09-22 時点で **307位 / 2,687チーム(上位11.4%)** と暫定的に圏内
  (15%のラインは 403位)。**参加チームは増え続けるため、同じスコアでも順位は下がる。**
  実際 9/21 の 266位 → 9/22 の 307位 と、スコア据え置きのまま41位下がっている。
- ~~当面の目標: CV AUC = 0.95~~ → **撤回**。較正した確率からラベルを再サンプリングして推定した
  「完璧なモデルのAUC」は **0.9459 ± 0.0003**。2位以下が 0.94675 で既に理論上限付近であり到達不能。
...
| 順位(2026-09-22 時点の暫定) | **307位 / 2,687チーム(上位11.4%)** |
| 提出構成 | **LightGBM 1/3 + XGBoost 1/3 + RealMLP 1/3**(rank平均) |
| LB1位 | 0.94945(2位は 0.94675。**1位だけ 0.0027 離れた外れ値**で public 過剰最適化の疑い) |
...
- **ノイズ床は 0.00015**(DeLong 導入後は 0.00003)。ただし **max_bin/border_count を動かす検証では
  0.00033 に上がる**ため、その場合は基準を引き上げること(調査元著者が1ヶ月間誤判定していた落とし穴)。
