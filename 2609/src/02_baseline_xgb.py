...
EDA の施策1「カテゴリ列を落とさずモデルに渡す」を、ここで検証する。
  1. 数値列のみ(7列)
  2. 数値列 + カテゴリ列(13列)。カテゴリはエンコードせず、XGBoost のネイティブなカテゴリ対応(enable_categorical)に渡す
...
def run_cv(feature_cols, cat_cols):
    ...
    for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y)):
        model = XGBClassifier(random_state=42, tree_method="hist", enable_categorical=True)
...
path = "submit/submission_baseline_xgb.csv"
