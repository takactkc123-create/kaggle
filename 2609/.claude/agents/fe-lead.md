...
|---|---|---|---|---|
| Count Encoding | 有効(+0.00083) | 有効(+0.00049) | **無効**(内部CTRと重複) | 未検証 |
| catify(低カーデ数値のカテゴリ化) | 未検証 | 未検証 | 有効(+0.00170) | 未検証 |
| Ordinal vs native category | native | Ordinal | cat_features | embedding |
| 入れ子TE | 適用済 | 適用済(+0.00108) | 適用済 | 要確認 |
| Smooth Keys | 適用済 | 適用済 | 適用済 | 要確認 |

**RealMLP は特に手薄**です。GBDT向けに開発されたFEがほとんど未検証で、最大の伸びしろがあります。
ただしNNは前処理の要求が違う(標準化必須、スケールに敏感、高カーディナリティのembedding)ため、
GBDTのFEをそのまま入れると悪化する可能性があります。**標準化の扱いに注意**すること。
...
- 通常 **±0.0002**、**ビン数(max_bin/border_count)を動かす検証では ±0.0004**(ノイズ床が上がるため)
