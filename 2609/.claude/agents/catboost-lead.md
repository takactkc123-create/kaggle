...
- `03_feature_engineering_catboost.py` — **特徴量エンジニアリング関数の定義のみ**。実行コードは書かない
- `04_train_and_evaluate_catboost.py` — `03_feature_engineering_catboost.py` を import して CV学習・評価・成果物出力を行う実行スクリプト
...
### 03_feature_engineering_catboost.py(関数群)
...
採用が決まって `03_feature_engineering_catboost.py` / `04_train_and_evaluate_catboost.py` の**列構成を変えたら**、
